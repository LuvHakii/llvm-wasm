...
const server = spawn(process.execPath, [resolve(import.meta.dirname, 'serve.ts'), DIST, String(PORT)], {stdio: 'inherit'});
...
try {
	...
	console.log('RUNNER ERROR:', String(e?.message ?? e).slice(0, 400));
	process.exitCode = 1;
} finally {
	await browser.close();
	server.kill();
